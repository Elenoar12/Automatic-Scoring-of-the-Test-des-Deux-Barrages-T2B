...
root.withdraw() #use to hide tkinter window
def search_for_file_path():
    filepath = filedialog.askopenfilename(parent = root, title = 'Please select a file', filetypes = (("Excel files","*.xlsm"),("all files","*.*")))
    return filepath
filepath = search_for_file_path()
excel_path = filepath
...
last_sym = pd.read_excel(excel_path, sheet_name='T2B').to_numpy()
last_sym = np.array(((last_sym[12][1] - 1) * 25 + (last_sym[12][2] - 1)))
...
gt = template.flatten()
gt[(last_sym + 1):] = 0                                                                                                 # last_sym + 1 = first symbol after the last_sym
gt = gt.reshape(40, 25)
