...
def symbol_splitter(x, frac_sym = 0.11):                                                                                #function to find row and clmn vals of single symbols:
  dim_sym = x.shape
...
def mean_method(sym_list):                                                                                              #determine symbol variations and areas outside the symbol:
  mean_out_areas = []
  mean_mark_list = []
...
def T2B_evaluator(jpeg_path, method, stop):
  ...
  vis = visual(img, mark, template, adapt_row, adapt_clmn)
  return eval, vis
...
excel_paths = search_for_xlsm_paths()
